...
class ControllerOcteamToolsCache extends Controller {
	...
	public function index() {
		...
		$data['action'] = str_replace('&amp;', '&', $this->url->link('octeam_tools/cache/remove', 'token=' . $this->session->data['token'], true));
		$data['back'] = $this->url->link('octeam/toolset', 'token=' . $this->session->data['token'], true);
		...
		$data['breadcrumbs'][] = array(
			...
			'href'  => $this->url->link('octeam_tools/cache', 'token=' . $this->session->data['token'], true)
		);
	...
	public function remove() {
		...
		if ($this->user->hasPermission('modify', 'octeam_tools/cache')) {
			...
			} elseif (isset($this->request->get['type']) &&  $this->request->get['type'] == 'system') {
